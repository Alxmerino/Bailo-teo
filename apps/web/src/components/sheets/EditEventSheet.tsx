...
import type { BailoteoEvent, BreastfeedData, BottleData, DiaperData, NoteData, PumpData, FeedSide, DiaperKind } from '@bailoteo/shared'
...
export default function EditEventSheet({ event, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    if (event.type === 'bottle') setOz(String(d.oz ?? ''))
  ...
  async function handleSave() {
    ...
    if (event.type === 'bottle') data = { oz: parseFloat(oz), notes: notes.trim() || undefined }
    ...
    <BottomSheet open={!!event} onClose={onClose} title="Edit Event" icon={<Pencil className="text-muted-foreground" size={20} />}>
      <div className="space-y-4">
        ...
        {(event.type === 'bottle' || event.type === 'pump') && (
          <div>
            <label className="mb-1.5 block text-sm font-medium text-muted-foreground">
              {event.type === 'pump' ? 'Amount (oz, optional)' : 'Amount (oz)'}
            </label>
            <input
              type="number"
              inputMode="decimal"
